...
extern int my_tid; // Used to store the MPI Rank for logging
...
// MPI Message Structs to replace PVM packing/unpacking
typedef struct {
    char board[64];
    int from;
    int to;
...
static void log_action(const char* fmt, ...) {
    ...
    fprintf(f, "[%s][Rank: %d] ", time_str, my_tid);
...
static void log_board_state(const char* board, const char* label) {
    ...
    fprintf(f, "\n[%s][Rank: %d] --- Chess Board State: %s ---\n", time_str, my_tid, label);
    for (int r = 7; r >= 0; r--) {
        fprintf(f, "[Rank: %d] %d | ", my_tid, r + 1);
        for (int c = 0; c < 8; c++) {
    ...
    fprintf(f, "[Rank: %d]     ----------------\n", my_tid);
    fprintf(f, "[Rank: %d]     a b c d e f g h\n\n", my_tid);
    fclose(f);
}
