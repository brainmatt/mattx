...
enum { MATTX_CMD_UNSPEC, MATTX_CMD_NODE_JOIN, __MATTX_CMD_MAX };
...
void trigger_kernel_join(uint32_t node_id, uint32_t ip_addr) {
    ...
    for (int i = 0; i < known_nodes_count; i++) {
        if (known_nodes[i] == node_id) {
            // printf("Node %u already known, skipping JOIN\n", node_id);
            // fflush(stdout);
            return; 
        }
    }
    ...
    if (nl_send_auto(nl_sock, msg) < 0) {
        ...
        fflush(stdout);
        if (known_nodes_count < 64) known_nodes[known_nodes_count++] = node_id;
    }
    nlmsg_free(msg);
}
...
int main() {
    ...
    while (1) {
        ...
        if (recvfrom(s, &beacon, sizeof(beacon), 0, (struct sockaddr*)&client_addr, &addr_len) > 0) {
            printf("DEBUG: Received beacon from Node %u (IP: %s)\n", beacon.node_id, inet_ntoa(client_addr.sin_addr));
            fflush(stdout);

            if (beacon.node_id == config.node_id) {
                printf("DEBUG: Ignoring own beacon\n");
                fflush(stdout);
            } else {
